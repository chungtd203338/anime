...
(function () {
    ...
    e.prototype.getCamelCasedButtonName = function (b) {
        var a = this.editorInstance.ui.items,
            d;
        for (d in a) if (a[d].name == b) return d;
        return null;
    };
    ...
    e.map = function (b, a) {
        ...
        if (CKEDITOR.tools.isArray(b)) {
            d = [];
            for (var c = b.length, f = 0; f < c; f += 1) d.push(a(b[f]));
    ...
    e.createToolbarButton = function (b) {
        ...
        if (b instanceof CKEDITOR.ui.button)
            ...
            a.addClass('cke_combo_button');
            d.addClass('cke_combo_text');
            d.addClass('cke_combo_inlinelabel');
            d.setText(b.label);
            c.addClass('cke_combo_open');
            f.addClass('cke_combo_arrow');
            c.append(f);
            a.append(d);
            a.append(c);
        }
    ...
    e.createIcon = function (b, a, d) {
        ...
        a.addClass('cke_button__' + b + '_icon');
    ...
    e.createButton = function (b, a) {
        ...
        if ('string' == typeof a) {
            a = a.split(' ');
            for (var c = a.length; c--; ) d.addClass(a[c]);
        }
        d.setHtml(b);
        return d;
    };
