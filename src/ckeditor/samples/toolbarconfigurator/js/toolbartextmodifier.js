(function () {
        ...
        a = CKEDITOR.tools.isArray(a)
            ? '\tconfig.toolbar \x3d ' +
              ('[\n\t\t' +
                  g
                      .map(a, function (a) {
                          return l.stringifyJSONintoOneLine(a, {
                              addSpaces: !0,
                              noQuotesOnKey: !0,
                              singleQuotes: !0,
                          });
    ...
    e.prototype._mapToolbarGroupsToToolbar = function (a, d) {
        ...
        d = 'string' == typeof d ? d.split(',') : [];
        for (var f = a.length; f--; ) {
            var c = this._mapToolbarSubgroup(a[f], d);
            'separator' === a[f].type
                ? (a[f] = '/')
    ...
    e.prototype.mapToolbarToToolbarGroups = function (a) {
        ...
        for (var f = {}, c = [], b = [], c = a.length, e = 0; e < c; e++)
            if ('/' === a[e]) b.push('/');
                ...
                m.name = a[e].name;
                m.groups = [];
                for (var l = g.length, p = 0; p < l; p++) {
                    var n = g[p];
                    if ('-' !== n) {
                        var h = this.getToolbarGroupByButtonName(n);
                        -1 === m.groups.indexOf(h) && m.groups.push(h);
                        f[h] = f[h] || {};
