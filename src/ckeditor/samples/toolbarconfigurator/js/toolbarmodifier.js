(function () {
        ...
        this.toolbarButtons = [
            {
                ...
                clickCallback: function (a, b) {
                    a[
                        a.hasClass('button-a-background')
                            ? 'removeClass'
                            : 'addClass'
                    ]('button-a-background');
            ...
            {
                text: 'Get toolbar \x3cspan class\x3d"highlight"\x3econfig\x3c/span\x3e',
                group: 'edit',
                position: 'right',
    ...
    d.prototype.getActualConfig = function () {
        var a = l.prototype.getActualConfig.call(this);
        if (a.toolbarGroups)
            for (var b = a.toolbarGroups.length, c = 0; c < b; c += 1)
                a.toolbarGroups[c] = d.parseGroupToConfigValue(
                    a.toolbarGroups[c],
                );
        return a;
    };
    ...
    d.prototype._createModifier = function () {
        ...
        var c = this.modifyContainer.find('li[data-type\x3d"group"]');
        ...
        for (var e = c.count(), f = 0; f < e; f += 1)
            c.getItem(f).on('mouseenter', a);
        CKEDITOR.document.on('keypress', function (a) {
            a = a.data.$.keyCode;
            a = 32 === a || 13 === a;
            var c = new CKEDITOR.dom.element(CKEDITOR.document.$.activeElement);
            c.getAscendant(function (a) {
                return a.$ === b.mainContainer.$;
            }) &&
                a &&
                'button' === c.data('type') &&
                c.findOne('input').$.click();
        });
        this.modifyContainer.on('click', function (a) {
            var c = a.data.$,
            ...
            if ((a = d.getGroupOrSeparatorLiAncestor(e))) {
                ...
                if (e.$ instanceof HTMLInputElement)
                    b._handleCheckboxClicked(e);
                else if (
                    e.$ instanceof HTMLButtonElement &&
                    (c.preventDefault
                        ? c.preventDefault()
                        : (c.returnValue = !1),
                    (c = b._handleAnchorClicked(e.$)) && 'remove' == c.action)
                )
                    return;
                c = a.data('type');
                a = a.data('name');
    ...
    d.prototype.showToolbarBtnsByGroupName = function (a) {
        if (this.toolbarContainer)
            for (
                var b = this.toolbarContainer.find('button'),
                    c = b.count(),
                    e = 0;
                e < c;
                e += 1
    ...
    d.parseGroupToConfigValue = function (a) {
        if ('separator' == a.type) return '/';
        var b = a.groups,
            c = b.length;
        delete a.totalBtns;
        for (var e = 0; e < c; e += 1) b[e] = b[e].name;
        return a;
    };
    d.getGroupOrSeparatorLiAncestor = function (a) {
        return a.$ instanceof HTMLLIElement && 'group' == a.data('type')
            ? a
            : d.getFirstAncestor(a, function (a) {
                  a = a.data('type');
    ...
    d.prototype._setActiveElement = function (a, b) {
        ...
        if (null === a)
            ...
            var c = this.mainContainer.findOne(
                'ul[data-type\x3dtable-body] li[data-type\x3d"' +
                    a +
                    '"][data-name\x3d"' +
                    b +
                    '"]',
            ...
            'group' == a && this._highlightGroup(b);
            'separator' == a && this._dehighlightActiveToolGroup();
        }
    };
    ...
    d.prototype._highlightGroup = function (a) {
        this.editorInstance.container &&
            ...
            a &&
                (a = d.getFirstAncestor(a, function (a) {
                    return a.hasClass('cke_toolbar');
                })) &&
                a.addClass('active'));
    };
    d.prototype.getFirstEnabledButtonInGroup = function (a) {
        var b = this.actualConfig.toolbarGroups;
        a = this.getGroupIndex(a);
        b = b[a];
        if (-1 === a) return null;
        a = b.groups ? b.groups.length : 0;
        for (var c = 0; c < a; c += 1) {
            var e = this.getFirstEnabledButtonInSubgroup(b.groups[c].name);
            if (e) return e;
        }
        return null;
    };
    d.prototype.getFirstEnabledButtonInSubgroup = function (a) {
        for (
            var b = (a = this.fullToolbarEditor.buttonsByGroup[a])
                    ? a.length
                    : 0,
                c = 0;
            c < b;
    ...
    d.prototype._handleCheckboxClicked = function (a) {
        var b = a.getAscendant('li').data('name');
        a.$.checked
    ...
    d.prototype._handleAnchorClicked = function (a) {
        ...
        var b = a.getAscendant('li'),
            c = b.getAscendant('ul'),
            e = b.data('type'),
            d = b.data('name'),
            g = a.data('direction'),
            m = 'up' === g ? b.getPrevious() : b.getNext(),
            h;
        ...
        if ('group' === e || 'separator' === e) h = this._moveGroup(g, d);
        'subgroup' === e &&
            ((h = b.getAscendant('li').data('name')),
            (h = this._moveSubgroup(g, h, d)));
        'up' === g && b.insertBefore(c.getChild(h));
        'down' === g && b.insertAfter(c.getChild(h));
        for (var k; (b = 'up' === g ? b.getPrevious() : b.getNext()); )
            if (this.emptyVisible || !b.hasClass('empty')) {
                k = b;
                break;
            }
        k ||
            ((k = '[data-direction\x3d"' + ('up' === g ? 'down' : 'up') + '"]'),
    ...
    d.prototype._refreshMoveBtnsAvalibility = function () {
        ...
        a(
            this.mainContainer.find(
                'ul[data-type\x3dtable-body] \x3e li \x3e ul',
            ),
        );
    };
    d.prototype._refreshBtnTabIndexes = function () {
        for (
            var a = this.mainContainer.find('[data-tab\x3d"true"]'),
                b = a.count(),
                c = 0;
            c < b;
            c++
    ...
    d.prototype.getGroupIndex = function (a) {
        for (
            var b = this.actualConfig.toolbarGroups, c = b.length, d = 0;
            d < c;
            d += 1
        )
    ...
    d.prototype._removeSeparator = function (a) {
        var b = CKEDITOR.tools.indexOf(
            this.actualConfig.toolbarGroups,
            function (b) {
                return 'separator' == b.type && b.name == a;
            },
        );
    ...
    d.prototype._determineSeparatorToAddIndex = function () {
        return this.currentActive
            ? ('group' == this.currentActive.elem.data('type') ||
              'separator' == this.currentActive.elem.data('type')
    ...
    d.prototype._moveElement = function (a, b, c) {
        function e(a) {
            return a.totalBtns || 'separator' == a.type;
        }
        c = this.emptyVisible
            ? 'down' == c
                ? b + 1
    ...
    d.prototype._moveSubgroup = function (a, b, c) {
        ...
        var d = CKEDITOR.tools.indexOf(b.groups, function (a) {
            return a.name == c;
        });
    ...
    d.prototype._calculateTotalBtns = function () {
        for (var a = this.actualConfig.toolbarGroups, b = a.length; b--; ) {
            var c = a[b],
                e = d.getTotalGroupButtonsNumber(c, this.fullToolbarEditor);
            'separator' != c.type && (c.totalBtns = e);
        }
    };
    d.prototype._addButtonToRemoved = function (a) {
        if (-1 != CKEDITOR.tools.indexOf(this.removedButtons, a))
    ...
    d.parseGroupToConfigValue = function (a) {
        ...
        var b = a.groups,
            c = b.length;
        delete a.totalBtns;
        for (var d = 0; d < c; d += 1) b[d] = b[d].name;
        return a;
    };
    d.getGroupOrSeparatorLiAncestor = function (a) {
        return a.$ instanceof HTMLLIElement && 'group' == a.data('type')
    ...
    d.getFirstAncestor = function (a, b) {
        for (var c = a.getParents(), d = c.length; d--; )
            if (b(c[d])) return c[d];
        return null;
    };
    d.getFirstElementIndexWith = function (a, b, c, d) {
        for (; 'up' === c ? b-- : ++b < a.length; ) if (d(a[b])) return b;
        return -1;
    };
    d.moveTo = function (a, b, c) {
        var d;
        -1 !== c && (d = b.splice(c, 1)[0]);
        a = c + a;
        b.splice(a, 0, d);
        return a;
    };
    d.getTotalSubGroupButtonsNumber = function (a, b) {
        var c = b.buttonsByGroup['string' == typeof a ? a : a.name];
        return c ? c.length : 0;
    };
    d.getTotalGroupButtonsNumber = function (a, b) {
        for (
            var c = 0, e = a.groups, f = e ? e.length : 0, g = 0;
            g < f;
            g += 1
        )
    ...
    d.prototype._getToolbarSubgroupString = function (a, b) {
        ...
        c += '\x3cul\x3e';
        for (var e = b ? b.length : 0, f = 0; f < e; f += 1)
            c += this.getButtonString(b[f]);
        return (c += '\x3c/ul\x3e\x3c/li\x3e');
    };
    d.prototype._getConfigButtonName = function (a) {
        var b = this.fullToolbarEditor.editorInstance.ui.items,
            c;
    ...
    d.prototype.getButtonString = function (a) {
        var b = this.isButtonRemoved(a.name) ? '' : 'checked\x3d"checked"';
        return [
    ...
    d.evaluateToolbarGroupsConfig = function (a) {
        return (a = (function (a) {
            ...
            return c.toolbarGroups && 'number' === typeof c.toolbarGroups.length
                ? JSON.stringify(c)
                : d && 'number' === typeof d.length
